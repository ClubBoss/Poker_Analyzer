...
class _FakeDecay extends SmartDecayGoalGenerator {
  ...
  @override
  Future<List<GoalRecommendation>> recommendDecayRecoveryGoals({int max = 5}) async => list;
}
...
class _FakeMistake extends SmartMistakeGoalGenerator {
  ...
  @override
  Future<List<GoalRecommendation>> recommendMistakeRecoveryGoals({int max = 5, int minMistakes = 3, double evLossThreshold = 1.0}) async => list;
}

void main() {
  test('interleaves decay and mistake recommendations', () async {
    final decay = _FakeDecay([
      const GoalRecommendation(tag: 'd1', reason: 'r', type: GoalRecommendationType.decay),
      const GoalRecommendation(tag: 'd2', reason: 'r', type: GoalRecommendationType.decay),
    ]);
    final mistake = _FakeMistake([
      const GoalRecommendation(tag: 'm1', reason: 'r', type: GoalRecommendationType.mistake),
    ]);
