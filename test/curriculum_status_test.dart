...
void main() {
  group('curriculum status', () {
    test(
      'modules_done follows SSOT order and starts with first canonical present',
      () {
        // 1) Load status
        ...
        expect(
          ...
          isTrue,
          reason: 'curriculum_status.json not found',
        );
        final statusJson =
        ...
        expect(
          modulesDone,
          isNotEmpty,
          reason: 'modules_done must be non-empty',
        );
        ...
        expect(
          ssotFile.existsSync(),
          isTrue,
          reason: 'tooling/curriculum_ids.dart not found',
        );
        ...
        if (listMatch != null) {
          ...
          ssotIds = RegExp(
            r"'([^']+)'",
          ).allMatches(ssotText).map((m) => m.group(1)!).toList();
        }
        ...
        for (final m in modulesDone) {
          ...
          expect(
            known,
            isTrue,
            reason: "Unknown module id in modules_done: '$m'",
          );
        }
        expect(
          modulesDone.toSet().length,
          modulesDone.length,
          reason: 'modules_done contains duplicates',
        );
        ...
        expect(
          sorted.first,
          equals(expectedStarter),
          reason:
              "modules_done must start with the first canonical ID present per SSOT ordering",
        );
        ...
        if (firstSpecialIdx != -1) {
          final anyCanonicalAfter = sorted
              .skip(firstSpecialIdx)
              .any((m) => rank.containsKey(m));
          expect(
            anyCanonicalAfter,
            isFalse,
