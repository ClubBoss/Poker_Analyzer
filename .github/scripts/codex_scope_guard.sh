#!/usr/bin/env bash
set -euo pipefail
# Разрешённые пути для авто-действий Codex
...
# Сравниваем с base (PR) или предыдущим коммитом (push)
...
# Если есть файлы вне ALLOW_RE — мягко выходим: шаги авто-действий не запускаем
if echo "$CHANGED" | grep -E -v "$ALLOW_RE" | grep -q '.'; then
  ...
  exit 78 # neutral, чтобы job/step пометился как пропущенный/neutral
fi
