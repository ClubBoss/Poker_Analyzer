...
class _TrainingPackSpotPreviewCardState
  ...
  // Упростил: избегаем copyWith - используем внешний onDuplicate/ onDupTap.
  void _duplicate() {
    // если передан кастомный обработчик - вызываем его
    if (widget.onDupTap != null) {
      widget.onDupTap!();
      return;
    }
    ...
    // запасной вариант - ничего не делаем (безопасно)
    debugPrint('Duplicate tapped (no handler provided).');
  }
                                    ...
                                    for (final tag in spot.tags)
                                      InputChip(
                                        ...
                                        onPressed: () => widget.onTagTap
                                            ?.call(tag.toLowerCase()),
                                      ),
                                  ],
                                ),
                              ),
                            if (actionCount > 0 ||
                                spot.note.trim().isNotEmpty)
                              Padding(
                                padding: const EdgeInsets.only(top: 4),
                                child: Row(
